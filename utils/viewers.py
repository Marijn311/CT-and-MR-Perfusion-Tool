...
def view_4d_img(img_list, title, projection=None, vmin=0, vmax=100):
    ...
    vmin and vmax are used to set the colorbar limits. so a vmax of 98 means that 98% of the data will be the max value of the colorbar.
     this improves contrast if there are outliers in the data.
    """
...
def show_perfusion_map(volume, title, vmin=0, vmax=100):
    """Visualize the perfusion map that were generated by the toolbox"""
...
def view_aif_selection(time_index, img_ctc, aif_propperties, aif_candidate_segmentations, brain_mask, mean_fitting_error, aif_smoothness, vmin=0, vmax=100):
    ...
    for s in range(img_ctc.shape[1]):  # Loop through slices
        ...
        for t in range(img_ctc.shape[0]):  # Loop through time
            ...
            if np.any(brain_mask_slice > 0):  # Only if there are brain voxels in this slice
                all_masked_values.extend(masked_image[brain_mask_slice > 0].flatten())
...
def show_comparison_maps(generated, commercial, title, mask, apply_mask):
    ...
    # We only use the pixels which exist in both masks.
    if apply_mask==True:
    ...
    img2 = ax2.imshow(commercial_masked[slice_idx], cmap=cmap, vmin=com_vmin, vmax=com_vmax)
